...
var MOBILE_BASE_URL = 'https://beta-api.lambdatest.com/api/v1';
...
for (var tunnel = 0; tunnel < LT_TUNNEL_NUMBER; tunnel++) {
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 15:
            ...
            _context2.next = 19;
            return _axios["default"].get(MOBILE_BASE_URL + "/device?sort=brand&real=true").then(function (res) {
              var iosDevices = res.data.ios;
              var androidBrands = res.data.android;
              var iosDeviceList = [];
              var androidDeviceList = [];
              iosDevices.map(function (item) {
                var osVersion = item.osVersion;

                if (item.deviceType === 'real') {
                  osVersion.map(function (version) {
                    if (version.isRealDevice === 1) iosDeviceList.push(item.deviceName + "@" + version.version + ":ios:isReal");
                  });
                }
              });
              androidBrands.map(function (item) {
                var androidDevices = item.devices;
                androidDevices.map(function (device) {
                  if (device.deviceType === 'real') {
                    var _osVersion = device.osVersion;
...
function _parseCapabilities2() {
  _parseCapabilities2 = (0, _asyncToGenerator2["default"])( /*#__PURE__*/_regenerator["default"].mark(function _callee5(id, capability) {
    var testcafeDetail, parseCapabilitiesData, browserName, browserVersion, platform, lPlatform, additionalCapabilities, _tunnel, _isRunning, rand;
      ...
      while (1) {
        switch (_context5.prev = _context5.next) {
          ...
          case 59:
            ...
            if (browserName && browserName.toLowerCase() === 'firefox' && browserVersion && browserVersion.split('.')[0] > 47 && !('enableCustomTranslation' in capabilities[id])) capabilities[id].enableCustomTranslation = true; // showTrace('Parsed Capabilities ', capabilities[id]);
    ...
    }, _callee5, null, [[0, 75], [13, 19], [30, 55]]);
  }));
  return _parseCapabilities2.apply(this, arguments);
}
